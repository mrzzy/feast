...
setup_retrieval_test() {
    pytest python/redis/basic-ingest-redis-serving.py --allow_dirty=true -k "test_basic_register_feature_set_success" $1
    pytest python/redis/basic-ingest-redis-serving.py --allow_dirty=true -k "test_basic_ingest_success" $1
    pytest python/redis/basic-ingest-redis-serving.py --allow_dirty=true -k "test_basic_retrieve_online_success" $1
}
...
then
    setup_retrieval_test $PYTEST_ARGS
    (cd go; env SERVING_HOST=localhost SERVING_PORT=6566 USE_AUTH=$ENABLE_AUTH go test)
fi
